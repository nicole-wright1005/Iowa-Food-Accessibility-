...
```{r subset atlas }
atlas_Polk <- atlas %>%
  select(GEOID, State, County, Urban, POP2010, PovertyRate, lahunv1share, lablack1share, lablackhalfshare, LILATracts_Vehicle, TractHUNV, TractLOWI, LILATracts_1And10, LowIncomeTracts, laseniorshalfshare, laseniors1share, laseniors1share, geometry) %>%
  filter(County == "Polk") %>%
  ...
  mutate(per_NoCar1mi = lahunv1share * 100,  
         ...
         seniors1 = cut((laseniors1share * 100), breaks = c(-Inf, 2.5, 5, 10),
                   labels = c("<2.5", "2.5-5", "5-10")))
